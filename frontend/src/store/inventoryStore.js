...
const useInventoryStore = create((set, get) => ({
  ...
  administrations: [],
  prescriptions: [],
  treatmentRecords: [], // New: AMU/Treatment records
  ...
  // Treatment Recording (New)
  addTreatmentRecord: (treatment) => {
    set((state) => ({
      treatmentRecords: [...state.treatmentRecords, {
        ...treatment,
        id: Date.now().toString(),
  ...
  // Get treatment records by animal
  getTreatmentsByAnimal: (animalId) => {
    return get().treatmentRecords.filter(record => record.animalId === animalId)
  },

  // Get animals in withdrawal period
  getAnimalsInWithdrawal: () => {
    const today = new Date()
    return get().treatmentRecords.filter(record => {
      const safeDate = new Date(record.safeDate)
